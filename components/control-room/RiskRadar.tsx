...
export function RiskRadar({ stages, agents }: RiskRadarProps) {
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-5 animate-fade-in space-y-4 shadow-sm">
      ...
      <div className="space-y-3">
        ...
        {blockedStages.map((stage) => (
          ...
          >
            ...
            <div>
              <p className="font-semibold text-red-700">작업 막힘</p>
        ...
        {userInputStages.map((stage) => (
          ...
          >
            ...
            <div>
              <p className="font-semibold text-amber-700">사용자 입력 대기</p>
        ...
        {problematicAgents.map((agent) => (
          ...
          >
            <AlertTriangle className="w-4.5 h-4.5 text-red-400 shrink-0 mt-0.5" />
            <div>
              <p className="font-semibold text-red-700">에이전트 경고: {agent.name}</p>
        ...
        {pendingQaStage && (
          <div className="flex items-start gap-3 rounded-lg border border-amber-500/20 bg-amber-500/5 p-3 text-xs">
            ...
            <div>
              <p className="font-semibold text-amber-700">QA / 검증 대기</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 border-t border-border/40">
          <div className="rounded-lg bg-zinc-50 p-2.5 flex items-center justify-between border border-zinc-200">
        ...
        {!hasRisks && (
          <div className="flex items-center gap-2.5 rounded-lg border border-emerald-200 bg-emerald-50 p-4 justify-center text-xs">
            <ShieldCheck className="w-4 h-4 text-emerald-400 shrink-0" />
            <span className="text-emerald-700 font-medium">모든 시스템 안정 · 감지된 위험 없음</span>
          </div>
        )}
      </div>
    </div>
